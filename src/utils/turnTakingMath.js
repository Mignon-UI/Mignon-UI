...
function getCachedParsedBio(botId, bio, otherNames) {
  ...
  const sortedOthers = [...otherNames].sort().join(',');
  const cacheKey = `${botId || ''}:${bio || ''}:${sortedOthers}`;
  let cached = bioCache.get(cacheKey);
  if (!cached) {
    cached = parseCharacterBio(bio, otherNames);
    bioCache.set(cacheKey, cached);
  }
  return cached;
}
...
export async function runMainThreadEfficientSelector(messageContent, bots, messages, sceneState = null) {
  ...
  if (lastMsg && lastMsg.sender_type === "character" && lastSpeakerName) {
    ...
    if (speakerBot) {
      const traitsS = getCachedParsedBio(speakerBot.id, speakerBot.personality, otherNames);
